...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 A build cancelled by mid-work re-validation (occupancy/site-vanished) emits a distinguishable failure event (not a bare agent.intent_done) — event type + payload documented in docs/wiki/event-types.md
- [x] #2 The builder receives a situated failure memory (origin action) stating the build did NOT complete and why, so the belief is falsifiable
- [x] #3 Transient occupancy no longer permanently kills a build: pathing avoids active build res tiles OR the build tolerates/waits out a passerby on the res tile (design choice recorded on this task)
- [x] #4 Regression test: helper agent paths onto the res tile mid-build; assert failure event + failure memory (or successful wall under the chosen tolerance design), never a silent bare intent_done
- [x] #5 Spec phase: Setup
- [x] #6 Spec phase: Foundational (Blocking Prerequisites)
- [x] #7 Spec phase: User Story 1 — Loud, distinguishable build failure (Priority: P1) 🎯 MVP
- [x] #8 Spec phase: User Story 2 — Builder remembers the failure (Priority: P1)
- [x] #9 Spec phase: User Story 3 — Passerby no longer kills the build (Priority: P2)
...
spec-bridge sync: Setup: 1/1 · Foundational (Blocking Prerequisites): 3/3 · User Story 1 — Loud, distinguishable build failure (Priority: P1) 🎯 MVP: 5/5 · User Story 2 — Builder remembers the failure (Priority: P1): 2/2 · User Story 3 — Passerby no longer kills the build (Priority: P2): 4/4 · Polish & Cross-Cutting: 3/3 — status In Progress → Done

ACs 1-4 verified against merged PR #68 (e1e3d66): #1 agent.build_failed event + event-types.md row; #2 same-tick situated OriginAction failure memory; #3 tolerance design (occupancy defers, wallOccupancyGraceTicks=120 bound) — choice recorded above; #4 TestWallBuildToleratesPasserby / rewritten TestWallOccupancyGuard / TestWallBuildDefersThenCompletes / TestWallBuildSiteVanishedFailsLoud + replay byte-identity.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
All spec tasks complete (Setup: 1/1 · Foundational (Blocking Prerequisites): 3/3 · User Story 1 — Loud, distinguishable build failure (Priority: P1) 🎯 MVP: 5/5 · User Story 2 — Builder remembers the failure (Priority: P1): 2/2 · User Story 3 — Passerby no longer kills the build (Priority: P2): 4/4 · Polish & Cross-Cutting: 3/3). Derived Done by spec-bridge sync.
