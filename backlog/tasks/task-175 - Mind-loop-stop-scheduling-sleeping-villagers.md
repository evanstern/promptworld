...
T007 soak evidence MEASURED @ tick 261994 / 3.032 game-days. Interim only in that the soak continues overnight to strengthen n; the SC-003 verdict itself is already unambiguous and more data can only reinforce it. SC-003 PASSES on every clause: (1) 'is asleep' agent.intent_rejected = 0 over 3.032 game-days = 0.0/game-day against the 31.2/game-day playtest-1 baseline (905 over 29 game-days); target was <=1/game-day, so a 100% reduction. (2) planner cog.thought submitted while asleep = 0 (target 0). (3) NON-VACUOUS — villagers genuinely slept: 75 agent.slept / 74 agent.woke edges, and the gate's own work is directly visible: 'asleep at dequeue' 35 rows (outcome=suppressed), 'cancelled in flight: agent slept' 24 rows (outcome=unusable), 'dead at dequeue' 1 row (suppressed). That is 59 planner round-trips prevented in 3 game-days, each of which would previously have become an 'is asleep' rejection. Outcome tagging matches the spec exactly: dequeue rows suppressed, in-flight rows unusable. (4) Dead-agent parity holds: 0 '% is dead' rejections alongside the 1 'dead at dequeue' suppression. Queries per specs/106-sleep-gated-planning/soak.md sections 1-3. Ready for Done at close-out via spec-bridge sync once tasks.md T007 is ticked on the bookkeeping branch.
<!-- SECTION:NOTES:END -->
