...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 First-occurrence events trigger one-line lessons in the feed; each lesson fires at most once (auto-retire, persisted seen-state)
- [x] #2 Lesson trigger taxonomy covers at minimum: suppression, gru attack, charge regen, order expiry, first death
- [x] #3 All pushed lessons are also reachable from the ? overlay
- [x] #4 Dedicated lesson row with dwell + UI pointer + one-active/spacing/decay; stage-defaulted per decision 5
- [x] #5 Prompting-lesson tier included in the minimum taxonomy
- [x] #6 Lesson strings skin-tokened, each naming its pull path; seen-state per-user
- [x] #7 Spec phase: Foundational (blocking prerequisites)
- [x] #8 Spec phase: User Story 1 — first encounter teaches itself, exactly once (P1) 🎯 MVP
- [x] #9 Spec phase: User Story 2 — the player's own prompting practice (P2)
- [x] #10 Spec phase: User Story 3 — always findable again; the row knows its place (P3)
- [x] #11 Spec phase: Polish & gates (same PR)
<!-- AC:END -->
...
spec-bridge sync: Foundational (blocking prerequisites): 3/3 · User Story 1 — first encounter teaches itself, exactly once (P1) 🎯 MVP: 6/6 · User Story 2 — the player's own prompting practice (P2): 2/2 · User Story 3 — always findable again; the row knows its place (P3): 4/4 · Polish & gates (same PR): 3/3 — status In Progress → Done

Shipped via PR #88, merge dfa73d7 (merge commit — design-page pins are main-history). Evidence per AC: 1-2 lessons_test.go SC-001 sweep + catalog predicates; 3 SC-002 catalog↔helpLessons equality test; 4 lesson row above guardian strip, dwell/one-active/spacing/decay + stage-defaulted (lessonRowDefault); 5 prompting tier (rejected tool call / custom charter / fuzzy order); 6 skin-tokened via lessonSkinResolve (052-contract default table until TASK-121 merges) + pull suffix; seen-state per-user lessons-seen.json.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
All spec tasks complete (Foundational (blocking prerequisites): 3/3 · User Story 1 — first encounter teaches itself, exactly once (P1) 🎯 MVP: 6/6 · User Story 2 — the player's own prompting practice (P2): 2/2 · User Story 3 — always findable again; the row knows its place (P3): 4/4 · Polish & gates (same PR): 3/3). Derived Done by spec-bridge sync.
