...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Consolidation output truncation is detected and retried with a larger budget (or input is bounded so output fits) — a 30-day-old villager's nightly consolidation succeeds
- [x] #2 Per-night consolidation acceptance is observable (telemetry/log summary), and sustained failure is loud, not silent
- [x] #3 A regression test covers consolidation at large memory volume (late-world shape), not just day-1 shape
- [x] #4 Spec phase: Detection + ladder helper
- [x] #5 Spec phase: Consolidation integration
- [x] #6 Spec phase: Narrator generalization
- [x] #7 Spec phase: Per-night observability
- [x] #8 Spec phase: Docs + grounding
<!-- AC:END -->
...
Delivered via PR #147 (merge commit, verified). The nightly consolidation blackout — playtest-1's worst finding (nights 20-29: 0/8 accepted, silent) — is fixed: parse-first truncation detection (Stop==StopMaxTokens + router-honesty guard), same-prompt retry with doubled budget (1024→2048→4096, clamped at exported llm.MaxTokenBudget, ≤2 retries), distinct 'truncated' rejection when exhausted, ladder generalized to narrator chapter+epilogue (800→1600→3200), per-night acceptance summary with WARNING escalation after ≥2 consecutive all-fail nights. Late-world day-29 regression fixture (70-memory buffer, 14 beliefs) pins the playtest failure shape. Zero new event types; additive omitempty Retries payload field byte-compatible at zero retries; parse.go zero-diff. Post-112 reconcile hand-merged the mind.go seam (spec-104 advance/sweep + spec-105 night-report coexist, full suite green post-move); 19 pin-line-only wiki conflicts re-pinned honestly. 21 wiki notes re-pinned in-branch, player docs 13/13 fresh.
