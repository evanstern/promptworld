...
- [ ] T001 Add the byte-identity pin: a fixture test in `internal/tui/tiles_identity_test.go`
...
- [ ] T002 [P] Add the legacy-terrain pin: extend `internal/worldmap/worldmap_test.go` with
...
- [ ] T003 Create `internal/tui/tiles.go`: the StyleToken table (every color currently in
...
- [ ] T004 [US1] Rewire `legendGlyphLine()` and the `?` overlay glyph walkthrough in
...
- [ ] T005 [US1] Rewire `renderMapGrid`'s `tile()` leaves in `internal/tui/views.go`
...
- [ ] T006 [P] [US1] Add the sweep tests in `internal/tui/tiles_test.go`: (a) no per-tile
...
- [ ] T007 [P] [US1] Add the round-trip test in `internal/tui/tiles_test.go`: register a
...
- [ ] T008 [US1] Verify: T001 byte-identity pin + entire `go test ./internal/tui/...`
...
- [ ] T009 [US2] `internal/worldmap/worldmap.go`: append `Marsh`, `Sand` TileKind values
...
- [ ] T010 [US2] Extend `internal/worldmap/worldmap_test.go`: T002's legacy Hash pin still
...
- [ ] T011 [US2] `internal/world/world.go` (+ the migrate implementation it points at):
...
- [ ] T012 [P] [US2] World/migration tests in `internal/world/`: migrate preserves terrain
...
- [ ] T013 [P] [US2] `internal/sim`: add Marsh/Sand to `passable` (`terrain.go:38`);
...
- [ ] T014 [US2] `internal/tui/tiles.go`: add the marsh row (`░`, material token 256:65,
...
- [ ] T015 Full-suite + build verification: `go build ./... && go vet ./... && go test ./...`
