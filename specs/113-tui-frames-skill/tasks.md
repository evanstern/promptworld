...
- [ ] T001 Create `.claude/skills/tui-frames/SKILL.md` with frontmatter matching
...
- [ ] T002 Body — **how to see a screen** (FR-002): read
      ...
      `--dump`.
- [ ] T003 Body — **the surface** (FR-003): `--list`; live render for off-matrix sizes; the
      four committed sizes each with the boundary it straddles; `--ansi`; `--interactive`
      for feel rather than layout.
- [ ] T004 Body — **the design loop** (FR-004): target frame first, implement until
      `--dump` matches, the frame diff is the review artifact.
- [ ] T005 Body — **authority vs evidence** (FR-005) and the **narrow-fallback caveat**
      (FR-006, from spec 112 FR-008).
- [ ] T006 Body — **the three repo traps** (FR-007): root read-only + worktree recipe; bare
      `git commit -F <file>`; merge-commit-only.
- [ ] T007 Add `.claude/skills/tui-frames/scripts/check-frames.mjs` (FR-008): dump to a temp
...
- [ ] T008 Run the guard on an unmodified tree — exits 0, `git status` clean afterwards.
- [ ] T009 **Negative control:** append a byte to one committed frame, confirm the guard
      exits nonzero and names that file, then restore. Record the observed output.
- [ ] T010 Confirm the skill is discoverable: it appears in the skills listing with its
      description, and `.claude/skills/tui-frames/` matches the `player-docs` layout.
- [ ] T011 Wiki-in-PR (spec 069): if the branch touches any file a `docs/wiki/` note lists in
...
- [ ] T012 `node scripts/check-merge-drift.mjs pr` exits 0 from the worktree.
