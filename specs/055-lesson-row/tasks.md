...
- [x] T001 [P] Per-user seen-state record in `internal/worlds/lessons.go`:
...
- [x] T002 [P] Tests in `internal/worlds/lessons_test.go`: missing/corrupt/
...
- [x] T003 Catalog skeleton in `internal/tui/lessons.go`: `lessonEntry` type
...
- [x] T004 [US1] Trigger projection in `internal/tui/lessons.go`: `lessonTriggers`
...
- [x] T005 [US1] Wire into the client in `internal/tui/tui.go`: load `LessonsSeen` at
...
- [x] T006 [US1] `x` key in `internal/tui/tui.go` global dispatch: dismiss the active
      lesson (record seen, start spacing); strict no-op when none active (documented
      fallthrough, keymap doctrine).
- [x] T007 [US1] Render the row in `internal/tui/views.go` + chrome budget in
      ...
      `lessonSkinResolve`.
- [x] T008 [US1] Done-signal clearing in `internal/tui/lessons.go`: catalog `done`
      predicates (v1: `first-order-expired` clears on `metatron.order_placed`);
      clearing records seen + starts spacing.
- [x] T009 [US1] Tests in `internal/tui/lessons_test.go`: SC-001 fixture sweep (each
...
- [x] T010 [P] [US2] Prompting-tier predicates + prose in `internal/tui/lessons.go`:
...
- [x] T011 [P] [US2] Tests in `internal/tui/lessons_test.go`: exactly-once sweep for
...
- [x] T012 [US3] Populate `helpLessons` from `lessonCatalog` at client init (boot-time
      ...
      (SC-002).
- [x] T013 [US3] Stage defaults in `internal/tui/views.go`/`layout.go`:
...
- [x] T014 [US3] Fold + narrow in `internal/tui/layout.go`/`views.go`: under height
...
- [x] T015 [US3] Render tests in `internal/tui/lessons_test.go` (or
...
- [x] T016 [P] Amend `docs/design/tui/panels/lesson-row.md`: `status: specified →
      shipped`, fill real renderer symbols in the control table, add sources, re-pin
      `verified_against`.
- [x] T017 [P] Amend `docs/design/tui/patterns/keymap.md`: move `x` from "New global
      keys (specified, unbuilt)" into the global-mode table; re-pin.
- [x] T018 Run the gates from the worktree and fix what they surface:
