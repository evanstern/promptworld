...
- [x] T001 Locate the world-genesis seam: where `promptworld new` creates the
...
- [x] T002 [US1] Change `defaultRefuelDyingBelow` 3600 → 10800 in
      `internal/sim/tuning.go` with a doctrine comment citing TASK-108 evidence
      (42 burnouts vs 8 builds).
- [x] T003 [P] [US1] Tests: refuel intent fires at 2.5 game-hours remaining
...
- [x] T004 [US1] Same-PR doc moves (FR-006):
...
- [x] T005 [US2] Seed one `sim.tuning_applied` event (full
      `defaultTuning()` set via `sim.NewTuningEvent`) in the world-genesis
      seam found in T001, ordered with the other genesis events.
- [x] T006 [US2] Verify the spec-048 boot seed against a pinned world: present
      ...
      if the proof fails).
- [x] T007 [P] [US2] Replay-independence test (SC-002): create a pinned state,
...
- [x] T008 [US2] Document the pre-057/migrated-world default-shift hazard where
...
- [x] T009 [US3] Proof matrix test `internal/sim/reflex_matrix_test.go`: cold
      ...
      reflex-only (SC-003).
- [x] T010 [US3] Fix any fire-adjacent surgical gap the matrix exposes (e.g.
...
- [x] T011 [US4] Write `specs/057-survival-reflex-gaps/audit.md`: every
