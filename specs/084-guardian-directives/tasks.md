...
- [ ] T001 Worktree already cut (`.worktrees/task-157`, branch
...
- [ ] T002 `internal/target/target.go`: exported `ParseLocus` (bare
...
- [ ] T003 `internal/sim` new file(s) (`plans.go` or split):
...
- [ ] T004 Reducer arms for all seven event types per
  ...
  FR-004/006/009)
- [ ] T005 `internal/sim/loop.go`: four `injectSocialWhitelist` entries
...
- [ ] T006 `internal/sim/executor.go` `stepEvents`: designation-
...
- [ ] T007 `internal/sim/miracles.go` `rebaseTicks`: active-directive
  `ExpiresTick` SHIFT arm + KEEP doc comments (data-model §10, FR-014)
- [ ] T008 Reducer/lifecycle test suites: per-arm validation tables,
...
- [ ] T009 [US2] `internal/tool/registry.go`: `place_designation` /
  ...
  (plan D7)
- [ ] T010 [US2] `internal/guardian`: designation handlers — locus parse
...
- [ ] T011 [US2] `internal/tui/tiles.go`: three registry rows (site /
...
- [ ] T012 [US2] `docs/design/tui/`: amend `panels/map.md` (+ every page
  `node scripts/check-tui-design.mjs --changed` names) — re-verify +
  re-pin same-PR (spec-047 gate; plan D11, FR-007)
- [ ] T013 [US2] End-to-end designation tests: place each kind through
...
- [ ] T016 [US3] Door/firewall suites: rejection table (dead target,
  ...
  scenarios 2/6, AC #4)
- [ ] T017 [US3] Composition proof (AC #7 / SC-003): integration test —
...
- [ ] T018 [US4] `internal/mind/context.go`: `renderDirective` +
...
- [ ] T019 [US4] `internal/sim/policy.go`: `directiveDecision` per
  ...
  research R13)
- [ ] T020 [US4] Reflex matrix suites (the `reflex_matrix_test.go`
...
- [ ] T021 [US4] Interruption-resume drive (SC-002 / AC #6): reflex-only
...
- [ ] T022 [US4] Reflex-only end-to-end (SC-001): issued structure-site
...
- [ ] T023 [US1] `internal/tool/registry.go` + `derive.go`:
...
- [ ] T024 [US1] `internal/guardian/survey.go` + `handleSurvey`
...
- [ ] T025 [US1] Survey suites (SC-005): byte-identity for identical
...
- [ ] T026 `internal/tui/grammar.go`: digest rows for all seven event
  types; `TestCatalogSweep` green (SC-007; contracts §3)
- [ ] T027 Full-suite pass: `go test ./...` including the spec-062
