...
- [ ] T004 Implement manifest parsing + `tool.Tool` synthesis in internal/bundle/manifest.go per contracts/bundle-manifest.md: strict JSON decode, param mapping to `tool.Param` kinds, name/description/charges/limits validation mirroring internal/tool/validate.go rules
- [ ] T005 [P] Unit tests for manifest parsing in internal/bundle/manifest_test.go: valid manifest, unknown keys rejected, name≠folder rejected, param rule violations, charges/limits bounds (rules T1/T2/T7 of contracts/boot-validation.md)
