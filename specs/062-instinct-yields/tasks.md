...
- [ ] T001 Yield state: per-agent last non-reflex intent-completion tick
...
- [ ] T002 Doctrine constants (`internal/sim/agents.go`): danger bands
...
- [ ] T003 Rung classification in `internal/sim/policy.go`: restructure
...
- [ ] T004 [US1] PREP gate: skip the PREP group when (tick − last non-reflex
      completion) < prepYieldTicks OR any need below its danger band; SURVIVAL
      group unconditioned.
- [ ] T005 [P] [US1] Tests: yield window suppresses prep post-planner
...
- [ ] T008 [US3] Frontier-search rung (reuse nearestFrontier) between the
      night ladder's last rung and terminal sleep, gated per FR-005/plan R6.
- [ ] T009 [P] [US3] Tests: cold night, nothing known/carried/choppable,
...
- [ ] T010 [US4] `internal/sim/thrash_regression_test.go`: the Sage-shape
