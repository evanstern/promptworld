...
- [X] T001 Add live carry headroom (free units + cap) to the miracle-capable
  digest's per-villager line (spec 059 assembly path), same snapshot as
  positions; dead villagers unchanged (FR-001).
- [X] T002 One-line headroom reference in the give_item gloss
  (internal/tool/derive.go), noting reject-whole (FR-002).

## Phase 2: Tests

- [X] T003 Digest arithmetic test (partially-filled inventory ⇒ correct
