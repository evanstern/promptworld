...
- [ ] T001 Add the per-agent unavailability mirror (asleep|dead) to `Mind`,
  ...
  precedent).
- [ ] T002 Dequeue gate at the top of `runPlan`: unavailable ⇒ no
  ...
  (FR-003).
- [ ] T003 Tests: SC-001 (slept-in-queue ⇒ zero runLoop invocations, one
...
- [ ] T004 Per-agent race-safe cancel slot: `runPlan` registers its call
  ...
  untouched.
- [ ] T005 Tests: SC-002 (mid-call `agent.slept` cancels, nothing lands,
...
- [ ] T006 Regression tests: SC-005 — `agent.woke` re-arms and the next
