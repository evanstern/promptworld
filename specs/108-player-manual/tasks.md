...
- [ ] **T001** Read `docs/wiki/cli-promptworld.md`, `cli-world-lifecycle.md`,
...
- [ ] **T002** Read `docs/wiki/instance-manager.md`, `world-forking.md`,
...
- [ ] **T003** Read `docs/wiki/world-save-directory.md`, `world-save-manifest-fields.md`,
...
- [ ] **T004** Read `docs/wiki/llm-preflight-detection.md`, `llm-provider-health.md`,
...
- [ ] **T005** Record the current pin for every source above — `verified_against:` for
  `docs/wiki/` paths, `git log -1 --format=%H -- <path>` for `docs/llm-providers.md`.
  This list is the source of the meta tags.

## Phase 2 — Pages

- [ ] **T006** Create `docs/player/command-reference.html`: canonical skeleton + verbatim
  ...
  FR-001..FR-005.
- [ ] **T007** Create `docs/player/world-files-reference.html` from T003: one entry per
  ...
  FR-006, FR-007.
- [ ] **T008** Create `docs/player/troubleshooting.html` from T004: one section per
  symptom in the player's own words, each with what's happening · what to check · what
  to do. Satisfies FR-008, FR-009.
- [ ] **T009** Cross-link the three pages to each other and to the teaching pages they
  complement, and add the back-link to `index.html` each existing page carries.
- [ ] **T010** Self-check every new page: exactly one `generated-by` tag; every `source`
...
- [ ] **T011** Edit `docs/player/index.html`: add a Reference section with the three
...
- [ ] **T012** `.claude/skills/player-docs/scripts/check-freshness.mjs`: add the three
  slugs to `EXPECTED_PAGES`; update the explanatory comment above it (currently
  "nine -> thirteen"). Satisfies FR-015.
- [ ] **T013** `.claude/skills/player-docs/SKILL.md`: add the three pages to the expected
...
- [ ] **T014** Correct the mapping table's `playing-via-metatron.html` row to the
...
- [ ] **T015** `node .claude/skills/player-docs/scripts/check-freshness.mjs --check`
  exits 0 and reports 16 fresh, 0 stale, 0 missing, 0 broken-ref. Satisfies SC-004.
- [ ] **T016** `git diff --stat origin/main` lists exactly the six intended files plus
  the spec dir; `git diff origin/main -- docs/player/` shows no existing page changed
  but `index.html`. Satisfies FR-011.
- [ ] **T017** Editorial pass for FR-016: read all three pages as a non-engineer. Any
