...
- [ ] **T001** Confirm the baseline is green on the branch: `go test ./...` passes before any
  change. Record the result; a pre-existing failure is a finding to surface, not to fix here.

## Phase 2: Foundational (blocking prerequisites)

- [ ] **T002** (FR-006) Add a per-villager inventory CONTENTS snapshot to the guardian mirror.
...
- [ ] **T003** (FR-006) Test: `internal/guardian` — after absorbing a batch that changes a
...
- [ ] **T004** (FR-001) Register `inspect_pack` in `internal/tool/registry.go`: Effect `Read`,
  Gate `None`, one required string param `villager`. Follow `survey_site`'s entry exactly for
  effect/gate/param style.
- [ ] **T005** (FR-001, FR-015) Append `inspect_pack` to `RosterGuardian` and
...
- [ ] **T006** (FR-002, FR-003) NEW `internal/guardian/pack.go`: `buildPackSheet` renders the
...
- [ ] **T007** (FR-001, FR-004, FR-005) In the same file, `handleInspectPack` — the
...
- [ ] **T008** (FR-004) Confirm — and assert in test — that `inspect_pack` consumes no charge,
...
- [ ] **T009** (SC-001, AC#1, AC#6) Test `internal/guardian/pack_test.go`: the **world-03
...
- [ ] **T010** (FR-003, SC-001) Test: two identical `inspect_pack` calls return byte-identical
  sheets.
- [ ] **T011** (FR-005) Test: an unknown name, and a dead villager's name, each return
  `read_ok` with a roster-naming miss — never an error verdict.
- [ ] **T012** (FR-002) Test: empty pack renders `0/24, 24 free` and the empty-pack line; a
...
- [ ] **T013** (FR-007) Carry the survival origin onto the turn's dispatch state: `turnDispatch`
...
- [ ] **T014** (FR-007) `turnDispatch` gains the look-first ledger: a `map[int]bool` of villager
...
- [ ] **T015** (FR-007, FR-008) Gate `handleVision`: when `d.survival` and the resolved target
...
- [ ] **T016** (FR-007, FR-008) Gate `handleMiracle` the same way for kinds `give_item` and
  `take_item` only. Other miracle kinds and `send_omen` stay ungated (contracts §2).
- [ ] **T017** (SC-002, AC#2) Test: on a survival-origin turn against the Cedar fixture, a
  `send_vision` at Cedar with no prior `inspect_pack(Cedar)` is `rejected_gate`, the reason
  names `inspect_pack`, and no event landed.
- [ ] **T018** (SC-002) Test: the same call AFTER `inspect_pack(Cedar)` lands normally — the
  gate is repairable within one turn.
- [ ] **T019** (FR-008) Test: the gate is per-villager — `inspect_pack(Hazel)` does not license
  a vision at Cedar.
- [ ] **T020** (FR-008) Test: on a NON-survival turn, `send_vision` is ungated and behavior is
  byte-identical to pre-feature.
- [ ] **T021** (FR-007) Test: `send_omen` on a survival turn is never gated.
...
- [ ] **T022** (FR-009) Add `take_item` to `internal/tool/registry.go`'s `miracleKinds`,
...
- [ ] **T023** (FR-009, FR-018) Add `ItemTakenPayload` (contracts §3) to `internal/sim` and
  register it in `internal/sim/payloads.go`.
- [ ] **T024** (FR-010, FR-011) Implement `applyItemTaken` in `internal/sim/miracles.go`
...
- [ ] **T025** (FR-012) Add the `take_item` arm to `BuildMiracleBatch`
...
- [ ] **T026** (FR-012) Give the operator's IPC `miracle` door parity
  (`internal/ipc/server.go`) so `take_item` reaches the same builder — argument validation
  mirroring `give_item`'s case.
- [ ] **T027** (AC#3, SC-003) Test `internal/sim`: Cedar at 24/24 with 20 wood + 4 planks;
...
- [ ] **T028** (FR-010) Test: `take_item(wood, 30)` against 20 carried is rejected whole — no
  charge spent, inventory untouched, reason names the actual count.
- [ ] **T029** (FR-011, SC-005) Test the conservation invariant: total units in
  (inventory + tile pile) is unchanged by a removal, for a plain kind, for spears, and for
  food.
- [ ] **T030** (FR-011) Test: removal merges into an existing pile on the tile (one pile per
  tile); spears leave most-worn-first; food keeps its spoilage batch.
- [ ] **T031** (FR-010) Test: dead villager, unknown kind, zero/negative qty each rejected at
  the door with an in-fiction reason and no charge spent.
- [ ] **T032** (FR-009) Extend the existing kind-vocabulary drift tests
...
- [ ] **T033** (FR-013, SC-004) Test: a landed `take_item` batch contains exactly the
...
- [ ] **T034** (FR-014) Test: an `inspect_pack` call emits a `cog.tool_call` record with a read
  verdict and lands NO world event; a gated refusal emits a record carrying the refusal reason.
- [ ] **T035** (FR-017) `internal/tui/digest.go`: add the `guardian.item_taken` line renderer
  and the subject resolver (contracts §5), mirroring the two `guardian.item_granted` entries.
- [ ] **T036** (FR-017) Test: the event renders a plain-language summary and does not fall
  through as an unknown type; the subject column attributes it to the affected villager.
- [ ] **T037** (FR-018) Test: a recorded `guardian.item_taken` replays to byte-identical state.

## Phase 7: Cross-cutting polish

- [ ] **T038** (FR-016) `internal/tool/derive.go`: add `guardianToolDesc` entries for
  `inspect_pack` (read heading, beside `survey_site`) and `take_item` (miracle-kind gloss).
- [ ] **T039** (FR-016) Amend `give_item`'s carry-headroom hint (`derive.go:259`) to name
...
- [ ] **T040** (FR-016) `internal/skin/skin.go`: add `skin.guardian.example_ask.inspect_pack`,
  following the `survey_site` example-ask entry.
- [ ] **T041** (FR-015) Update `internal/guardian/stage_test.go`'s enumerated granted rosters to
  include `inspect_pack` and NOT `take_item`, and confirm the ceiling test fails if either is
  wrong.
- [ ] **T042** Run `go test ./...` and `go vet ./...`; both clean.
- [ ] **T043** (SC-006) Run the TUI design gate if `internal/tui/` changed:
