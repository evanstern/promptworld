...
- [x] T001 Create worktree `.worktrees/task-116` from fresh origin/main; confirm
      `go build ./... && go test ./internal/tui` green at base

## Phase 2: Foundational (blocking prerequisites)

- [x] T002 Extract the map legend into a shared glyph table (`glyphEntry` rows) in
...
- [x] T003 [P] Add `helpOpen`/`helpSection`/`helpTier`/`helpMode`/`helpScroll` fields
...
- [x] T004 [US1] Create internal/tui/help.go: per-mode key tables (global/home,
      ...
      footer-hinted set)
- [x] T005 [US1] Overlay key dispatch at the head of `handleKey` (after ctrl+c guard
...
- [x] T006 [US1] `helpPanelView` body replacement in `widescreenView` (solo-zoom
      ...
      help.go (R2, R4)
- [x] T007 [P] [US1] Footer hints gain `· ? help` in every `footerView` branch
...
- [x] T008 [US1] Tests in internal/tui/help_test.go + extensions: `?` opens from every
...
- [x] T009 [US2] Walkthrough content in help.go: `headerAnatomy` rows (every headerView
...
- [x] T010 [US2] Tests: content-presence assertions (views_test.go style) — every
...
- [x] T011 [US3] Tests: overlay renders and navigates with nil `status` and nil
...
- [x] T012 [US4] `helpLesson{id,title,body}` table (empty) + contract comment
      referencing contracts/help-content.md; lessons section renders entries or the
      placeholder line — internal/tui/help.go
- [x] T013 [US4] Test: inject a fixture lesson entry in-test → it renders and is
...
- [x] T014 Full gate: `go test ./...`, gofmt/vet; SC-005 soak test (open/close loop
      causes zero model-state diff outside help fields)
- [x] T015 Rebase over main; if spec 044's TUI changes (ENDED token, grave glyph,
      ...
      collision)
- [x] T016 [P] Run quickstart.md live walkthrough (§2–§4); record outcomes in the PR
