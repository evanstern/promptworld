...
- [X] T001 Cut the worktree per protocol (from repo root:
...
- [X] T002 [US1] Parser + classifier in `internal/tui/mouseparity_test.go`
...
- [X] T003 [US1] Oracle + live-dispatch proof in the same file: one entry —
...
- [X] T004 [US1] `TestMouseParitySweep` assembly: direction 1 (claim without
...
- [X] T005 [US1] Proof: `go test ./internal/tui/ -run TestMouseParity -v`
...
- [X] T006 [US2] Amend doctrine rule 3 in
...
- [X] T007 [US2] Re-verify + re-pin `docs/design/tui/patterns/keymap.md`
...
- [X] T008 Wiki probe: no note pins the touched files (spec Assumptions;
  ...
  clean (constitution IV, spec SC-004). Deviation found: no wiki note pins
  the touched files (confirmed) and `check-merge-drift.mjs pr` never reports
  `wiki-repin-missing`, BUT the separate `player-docs` skill's own
  `check-freshness.mjs --check` independently tracks
  `docs/design/tui/patterns/keymap.md` as a plain-file source for
  `docs/player/keys-reference.html` (not a `docs/wiki/` note) and reported
  it stale (exit 1) once the pin moved — this trigger isn't gated by
  "`docs/wiki/` changed" the way the task text assumed. Fixed in-branch:
  re-pinned `keys-reference.html`'s `keymap.md` source meta tag to the
  branch commit that landed the amendment; no prose change needed (the
  amendment is engineering-process-only, no new player-facing fact).
  `check-freshness.mjs --check` now exits 0 (13 fresh, 0 stale).
- [X] T009 Full-suite + pr gate: `go test ./...` green; `gofmt -l` clean on
  ...
  the keymap.md pin is a branch hash). Local half done (build+gates green,
  see findings); PR open/merge is the orchestrator's.
