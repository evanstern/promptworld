...
- **SC-002 (tables/tokens)**: `node scripts/check-tui-design.mjs` covers table
  ...
  `patterns/skin-tokens.md`'s default-skin value column, `INDEX.md`'s verbatim
  TASK-34 history blockquote, and backtick-quoted Go code identifiers (e.g.
  `metatronView`, `sim.MetatronChargeCap` — renderer/data-source columns name real
  symbols per the control-table contract; they are not fiction prose). Fiction
  appears only as tokens elsewhere.
