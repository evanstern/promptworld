...
- [ ] T001 Intent condition fields (plan R1): `UntilNeed`/`UntilValue`
...
- [ ] T002 Doctrine constants (`internal/sim/agents.go`): `warmthRecoverTo`
...
- [ ] T003 [US1] Executor hold-at-target + per-tick completion check
...
- [ ] T004 [US1] `warm_up` planner tool (`internal/tool/registry.go` +
      ...
      `warmthRecoverTo`.
- [ ] T005 [US1] Reflex issuance (`internal/sim/policy.go`): the 062 day and
...
- [ ] T006 [P] [US1] Tests: recover-then-release determinism (SC-001, exact
...
- [ ] T007 [US3] Abort-on-no-progress (plan R4): while holding, no net gain
      over `recoveryStallTicks` → distinct abort outcome; agent re-decides.
- [ ] T008 [P] [US3] Tests: fire-dies-mid-recovery aborts; preemption path
...
- [ ] T009 [US2] Rest analog (plan R6): align sleep's end conditions to the
...
- [ ] T010 [P] [US2] Tests: shared fields/check for the second consumer;
...
- [ ] T011 [US4] Cold-emergency wake arm in the wake gate
      (`internal/sim/executor.go`, plan R5) reusing the exposure-band
      constants; cozy sleep untouched.
- [ ] T012 [P] [US4] Tests: the Oak-final-night shape wakes the sleeper;
...
- [ ] T013 Extended Sage scenario (SC-002): the 062 thrash regression
...
- [ ] T014 Full gates in worktree: `go test ./...`, `go vet ./...`,
