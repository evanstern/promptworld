...
// Directories under docs/design/tui/ whose contents are GENERATED, not
// authored — every check below skips them wholesale (spec 112 T014).
//
// The taxonomy checks (file-set, pins, anatomy) all assert properties of an
// authored design page: it declares a class matching its directory, it pins
// the commit a human verified it against, and anatomy.md maps it. A
// generated artifact has none of those properties and should not: its
// authority is the generator, and re-pinning it by hand would be a claim
// nobody made. `frames/` is the frame harness's matrix — one plain-text
// frame per (fixture, state, size), regenerated by `promptworld frames
// --dump` — plus the authored README.md that says so. The frames themselves
// are `.txt` and were already invisible here (only `.md` is walked); this
// entry is what keeps that README from being rejected as an untaxonomized
// page.
const GENERATED_DIRS = new Set(['frames']);
...
// walkMarkdown collects every authored .md file under dir, skipping the
// generated directories (GENERATED_DIRS) at the top level. Skipping at
// collection time rather than inside each check is deliberate: it means a
// later check cannot accidentally reach a generated file by forgetting the
// exemption.
function walkMarkdown(dir, depth = 0) {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      if (depth === 0 && GENERATED_DIRS.has(entry.name)) continue;
      out = out.concat(walkMarkdown(abs, depth + 1));
