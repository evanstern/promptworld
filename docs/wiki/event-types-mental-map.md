...
description: Perception and mental-map event rows split from [[event-types]]: agent.moved, agent.saw, agent.map_corrected, social.place_told, guardian.place_revealed. Load when tracing spec 041's per-agent spatial knowledge — perception sweeps, told directions, corrected/stale facts, and vision-granted places.
kind: concept
sources:
