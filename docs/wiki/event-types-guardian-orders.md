...
description: Guardian standing-order event rows split from [[event-types]]: guardian.charge_regenerated/nudged/order_placed/order_triggered/order_cancelled/order_expired. Load when tracing charge regen, nudge validation, or the spec 029/059 standing-order and survival-watch lifecycle.
kind: concept
sources:
...
references are named refs"). `guardian.order_placed` now rides the wire as the
`OrderPlacedPayload` mirror — `GuardianOrder`'s fields with
`Agent AgentRef` (−1 = any, empty name) — while the state entity keeps the
bare int; the arm folds `.ID` (`internal/sim/guardian.go`).
...
order's lifecycle — `guardian.order_placed` (monitor_and_act), one-shot
`guardian.order_triggered` (the trigger worker, live-only, never replayed),
`guardian.order_cancelled` (cancel_order), and executor-emitted
`guardian.order_expired` (the `charge_regenerated` pattern: a pure function
...
`send_vision`, so `guardian.nudged`'s `form` domain is now `vision` (exactly
...
three watches ride the EXISTING `guardian.order_placed`/`order_cancelled`/
