...
emission class, no whitelist entries; [[scenario-machinery]]'s spec-054
rubric machinery is the production emitter for `first-night`, fixtures still
the only in-tree emitter for exercises whose rubric evaluator is unbuilt),
the
daemon's always-on unlock observer in `internal/daemon/curriculum.go`, and
the per-user unlocks record it projects. [[scenario-machinery]] also owns
`GuardianOrder.PlacedSeq` (spec 054) as an `OrderPlacedEvidence` consumer,
reduced alongside the rest of `applyGuardian`. [[sim-state-reducer]] owns the
`PairTalks` field, its `agent.talked`-arm write, and the `rebaseTicks` SHIFT
(spec 061); [[sim-loop]] owns the `rungPairCooldown` gate and the `set_plan`
clamp decision (spec 061/058); [[social-fabric]] owns the mind-side novelty
