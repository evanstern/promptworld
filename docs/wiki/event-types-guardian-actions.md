...
description: Guardian miracle-action and gru event rows split from [[event-types]]: guardian.time_snapped/item_granted/item_taken (spec 116)/entity_moved/entity_removed, guardian.region_named (spec 101, the canonization miracle), the gru.* family. Load when tracing miracle cost/gratis mechanics, the canonization miracle's charge shape, or the gru antagonist's emergence/attack lifecycle including the spec 044 escalated-kill path.
kind: concept
sources:
...
| `guardian.item_taken` | `ItemTakenPayload{agent, kind, qty, gratis}` | angel's turn reply or the CLI/IPC door, injected | spec 116, `give_item`'s mirror image: validates a living villager, a known item kind (the same `tool.GrantKinds()` set, rejection enumerating it as `takeable`), positive qty, and a carried-quantity check that rejects WHOLE naming what they actually hold; spends 1 charge unless `gratis`; MOVES `qty` units out of the pack into the pile on the villager's own tile (`pileFor`, create-or-merge — the one miracle arm that can mint a `Pile`), spears/axes leaving most-worn-first and food stamped with a fresh `SpoilAt` rot window (carried food carries none of its own) |
