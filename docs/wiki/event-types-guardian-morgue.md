...
references are named refs"). `morgue.epilogue`'s agent (−1 = the run-end
epilogue) and `chronicle.entry`'s agent list are named refs; the chronicle
arm folds `.ID`s into the state ring.
...
`run.ended`, the guardian turn pipeline injects `guardian.charter_observed`
