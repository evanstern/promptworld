...
- **`guardian.order_placed`** (payload = the whole `GuardianOrder`) — landed through
...
- **`guardian.order_triggered`** (`OrderTriggeredPayload{id, matched_type, matched_tick}`)
  ...
  event.
- **`guardian.order_cancelled`** (`OrderIDPayload{id}`) — injected by a `cancel_order`
  call.
- **`guardian.order_expired`** (`OrderIDPayload{id}`) — **executor-emitted**, a pure
  function of `(state, tick)` exactly like `guardian.charge_regenerated`: the [[executor]]
...
`guardian.order_expired`. [[scenario-machinery]] is the spec-054 consumer of
