...
size_budget_exempt: pre-existing overage (8056 chars before this pass) — TASK-196 added the `"pack full"` reason to the `agent.intent_failed` row, which must stay on this note as the normative event/payload table; a per-event-family summary-style split is a dedicated future pass, not this task's scope
---
...
| `agent.intent_failed` (spec 096) | `IntentFailedPayload{agent, goal, reason, x, y}` | executor, generalizing `agent.build_failed` — the mid-work `valid` exit (`forage`/`chop`/`hunt`/`demolish`/`repair`/`quarry`/`cook`/`bathe`, `"target gone"`), the completion-time no-op recheck (`craft_*`/`cook`/`bathe`/`deposit`/`withdraw`, `"contested"`, or `"invalid"` for `deposit`'s empty `Kind`), and — since TASK-196 — the zero-space guard on every gather goal (`freeBulk` zero at completion, `"pack full"`; previously a bare `agent.intent_done`) | intent cleared, identical to `build_failed`'s effect; paired memory (`salIntentFailed` = `salShelter`); mind re-arms the same way; `x`/`y` are the actor's own stand tile (no shared Target-vs-Res convention here); TUI digest renders a failure line |
